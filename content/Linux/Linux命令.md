...
| 项目                | `ps -ef`       | `ps aux`           |
| ----------------- | -------------- | ------------------ |
| 命令风格              | System V（Unix） | BSD（类 Unix）        |
| 是否跨平台             | 是（兼容性高）        | 否（某些系统可能不支持 `aux`） |
| 是否显示资源            | 否（不含 CPU/MEM）  | 是（含 %CPU, %MEM）    |
| 是否显示父子            | 是（含 PPID）      | 否                  |
| 使用场景              | 查父子结构、启动命令、用户  | 查资源占用、进程状态监控       |
...
- `ps -ef --forse`：常用！
	- 基本同 `ps -ef f`，对于长命令 `f` 会截断过长的部分而更美观；`--forse` 则会显示整条命令。
- `ps -ef`
- `ps -Cf`
	- `-e`：所有进程
